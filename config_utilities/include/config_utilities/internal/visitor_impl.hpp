...
void Visitor::visitField(T& field, const std::string& field_name, const std::string& unit) {
  ...
  if (visitor.mode == Visitor::Mode::kSet) {
    ...
    error.clear();  // We don't care about setting up the intermediate to get data.
    ...
    const T value = Conversion::fromIntermediate(intermediate, error);
    if (error.empty()) {
      // Make sure current value isn't lost if conversion fails.
      field = value;
    } else {
      visitor.data.errors.emplace_back(new Warning(field_name, error));
    }
